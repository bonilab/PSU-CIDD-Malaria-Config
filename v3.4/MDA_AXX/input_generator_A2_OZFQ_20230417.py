                                ...
                                if event['name'] == 'modify_nested_mft_strategy':
                                    if strategy == 'AAB':
                                        ...
                                        if strategy == 'ABA':
                                            ...
                                            new_data['mda_therapy_id'] = mda_therapy_id
                                            new_data['events'][index]['info'][0]['strategy_id'] = mda_therapy[0]
                            output_filename = 'A2_OZFQ_20230417/%d.yml'%(len(params))
                            output_stream = open(output_filename, 'w')
                            yaml.dump(new_data, output_stream)
                            output_stream.close()
                            params.append((mda_round,tc,beta,pfpr,cr,mda_coverage,strategy,mda_therapy[1]))
params_df = pd.DataFrame(params, columns=['mda_round','tc','beta','pfpr0','cr_factor','mda_coverage','strategy','b_drug'])
params_df.to_csv('A2_OZFQ_20230417_params.csv', index=True)
